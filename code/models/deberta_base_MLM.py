...
# Model
# ====================================================
class DebertaBaseLastMLM(nn.Module):
...
#         [self._init_weights(self.fc1[i]) for i in range(self.cfg.num_last)]
    ...
    def forward(self, inputs):
        ...
        feature = [
            self.relu(self.fc1(self.fc_dropout1_1(outputs[1][-1])).unsqueeze(-1)),
            self.relu(self.fc1(self.fc_dropout1_2(outputs[1][-1])).unsqueeze(-1)),
            self.relu(self.fc1(self.fc_dropout1_3(outputs[1][-1])).unsqueeze(-1)),
            self.relu(self.fc1(self.fc_dropout1_4(outputs[1][-1])).unsqueeze(-1))
        ]
        feature = torch.cat(feature,axis = -1).mean(axis=-1)
        
        output = self.fc2(self.fc_dropout2(feature))
        return output